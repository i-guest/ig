...
namespace UnityEngine.UI.Extensions
{
	...
	[ExecuteInEditMode]
	public class TextPic : TMP_Text, IPointerClickHandler, IEventSystemHandler, IPointerExitHandler, IPointerEnterHandler, ISelectHandler
	{
		[Serializable]
					...
					if ((image.sprite == null || image.sprite.name != value) && inspectorIconList != null && inspectorIconList.Length != 0)
					{
						for (int num = 0; num < inspectorIconList.Length; num++)
						{
							if (inspectorIconList[num].name == value)
							{
								...
								image.rectTransform.sizeDelta = new Vector2(base.fontSize * ImageScalingFactor * inspectorIconList[num].scale.x, base.fontSize * ImageScalingFactor * inspectorIconList[num].scale.y);
		...
		protected override void OnPopulateMesh(VertexHelper toFill)
		{
			...
			for (int i = 0; i < m_ImagesVertexIndex.Count; i++)
			{
				...
				if (num < toFill.currentVertCount)
				{
					...
					positions.Add(new Vector2(vert.position.x + base.fontSize / 2f, vert.position.y + base.fontSize / 2f) + imageOffset);
