...
namespace UnityEngine.UI.Extensions
{
	...
	public class UILineRenderer : UIPrimitiveBase
	{
		...
		public Vector2 CalculatePointOnCurve(float t, Vector2 p1, Vector2 p2, Vector2 p3, Vector2 p4)
		{
			float num = t * t;
			float num2 = num * t;
			float x = p1.x + ((0f - p1.x) * 3f + t * (3f * p1.x - p1.x * t)) * t + (3f * p2.x + t * (-6f * p2.x + p2.x * 3f * t)) * t + (p3.x * 3f - p3.x * 3f * t) * num + p4.x * num2;
			float y = p1.y + ((0f - p1.y) * 3f + t * (3f * p1.y - p1.y * t)) * t + (3f * p2.y + t * (-6f * p2.y + p2.y * 3f * t)) * t + (p3.y * 3f - p3.y * 3f * t) * num + p4.y * num2;
		...
		protected override void OnEnable()
		{
			...
			goto IL_002d;
			IL_002d:
			if (base.transform.GetComponent<RectTransform>().position != Vector3.zero)
			{
