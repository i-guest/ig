	...
	public class RangeSlider : Selectable, IDragHandler, IEventSystemHandler, IInitializePotentialDragHandler, ICanvasElement
	{
		...
		private void UpdateVisuals()
		{
			...
			if (m_FillContainerRect != null)
			{
				...
				zero[(int)axis] = NormalizedLowValue;
				one[(int)axis] = NormalizedHighValue;
			...
			if (m_LowHandleContainerRect != null)
			{
				...
				Axis index = axis;
				float value = (one2[(int)axis] = NormalizedLowValue);
			...
			if (m_HighHandleContainerRect != null)
			{
				...
				Axis index2 = axis;
				float value = (one3[(int)axis] = NormalizedHighValue);
				zero3[(int)index2] = value;
		...
		private float CalculateDrag(PointerEventData eventData, Camera cam, RectTransform containerRect)
		{
			...
			if (rectTransform != null && rectTransform.rect.size[(int)axis] > 0f)
			{
				...
				return Mathf.Clamp01((localPoint - m_Offset)[(int)axis] / rectTransform.rect.size[(int)axis]);
			}
			return 0f;
		}
		...
		private void CalculateBarDrag(PointerEventData eventData, Camera cam)
		{
			...
			if (!(fillContainerRect != null) || !(fillContainerRect.rect.size[(int)axis] > 0f) || !RectTransformUtility.ScreenPointToLocalPointInRectangle(fillContainerRect, eventData.position, cam, out var localPoint))
			{
				return;
			}
			...
			if (NormalizedLowValue >= 0f && NormalizedHighValue <= 1f)
			{
				...
				float num2 = Mathf.Clamp01((localPoint - m_Offset)[(int)axis] / fillContainerRect.rect.size[(int)axis]) - num;
		...
		public override void OnPointerDown(PointerEventData eventData)
		{
			...
			{
				if (RectTransformUtility.ScreenPointToLocalPointInRectangle(m_FillRect, eventData.position, eventData.pressEventCamera, out var localPoint))
				{
					m_Offset = localPoint;
				}
		...
		{
			if (RectTransformUtility.ScreenPointToLocalPointInRectangle(transform, eventData.position, eventData.pressEventCamera, out var localPoint))
			{
				m_Offset = localPoint;
			}
		...
		public void SetDirection(Direction direction, bool includeRectLayouts)
		{
			...
			if (includeRectLayouts && this.axis != axis)
			{
