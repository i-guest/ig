...
namespace DG.Tweening
{
	...
	public class DOTweenPath : ABSAnimationComponent
	{
		...
		public Vector3[] GetDrawPoints()
		{
			...
			{
				if (path.nonLinearDrawWps != null)
				{
					return path.nonLinearDrawWps;
				}
				if (path.wps != null)
				{
					Path.RefreshNonLinearDrawWps(path);
					return path.nonLinearDrawWps;
				}
			}
			Debugger.LogWarning("Draw points not ready yet (path not initialized) or path tween has been killed. Returning NULL");
			return null;
		}
